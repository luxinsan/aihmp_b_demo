const patientProfile = await fetch("./shared/mock/patient-profile.json", { cache: "no-store" }).then((response) =>
  response.json(),
);
const staticWorkspaceData = await fetch("./shared/mock/static-workspace.json", { cache: "no-store" }).then((response) =>
  response.json(),
);
