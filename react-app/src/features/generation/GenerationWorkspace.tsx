...
function decorateEditableBlock(block: HTMLElement) {
...
function parseLogPresentation(entry: GenerationSession["logs"][number]) {
...
function getLogStatusIcon(kind: string) {
  switch (kind) {
    case "done":
      return "✓";
    case "system":
...
function buildLogGroups(logs: GenerationSession["logs"]) {
  const groups: Array<{
    id: string;
    tone?: GenerationSession["logs"][number]["tone"];
    kind: string;
    label: string;
    source: string;
    stage: string;
    status: GenerationSession["logs"][number]["status"];
    time: string;
    bodies: string[];
  }> = [];
  ...
  logs.forEach((item, index) => {
    ...
    if (canMerge) {
      ...
      previousGroup.time = item.time || previousGroup.time;
      return;
    }
...
export function GenerationWorkspace({
  ...
  function formatLogTime(index: number) {
    const offset = Math.max(session.logs.length - index - 1, 0) * 2 * 60 * 1000;
    return new Intl.DateTimeFormat("zh-CN", {
      hour: "2-digit",
      minute: "2-digit",
    }).format(Math.max(session.updatedAt - offset, session.createdAt));
  }
        ...
        <aside className="review-sidebar">
          ...
          <section className="review-card log-card">
            ...
            <ul className="process-log" id="processLog">
              {buildLogGroups(session.logs).map((entry) => (
                <li
                  className="log-entry"
                ...
                >
                  ...
                  <div className="log-entry-body">
                    <div className="log-entry-head">
                      ...
                      <strong className="log-entry-time">{entry.time || formatLogTime(0)}</strong>
                    </div>
