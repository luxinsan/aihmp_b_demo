...
- `patient-mobile-h5`：患者端移动 H5 主实现，负责患者端页面、状态栏、标题栏、微信胶囊、tab、组件、样式、mock 和页面交互。
- `patient-web-preview`：外部预览壳，只负责手机桌面、微信入口、真机外壳和 iframe 承载。
- `react-app`：医护端（B 端），不参与患者端移动页面渲染。

## 已删除的旧链路

- `patient-miniapp` 已删除。
- `5175`、`dev:h5`、`build:weapp` 已退出当前仓库主链路。
- 新增患者端需求不再进入旧 Taro 链路。

## 改动归属

改患者端页面、状态栏、标题栏、微信胶囊、tabBar、页面组件、页面 mock：
...
`健康计划` 和 `我的` 两个患者端 tab 后续会整体重做。当前只保留轻量演示骨架，短期不继续扩展页面细节。
